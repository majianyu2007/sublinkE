...
### feat
- 自动订阅任务支持 HTTP/HTTPS 代理节点、任务级镜像同步与目标订阅合并
- 自动同步保留手动节点，并在下载、解析或空订阅异常时保留上一版节点
- 节点管理页可配置目标订阅并查看镜像节点数量
- 一个镜像订阅组可同时分发到多个订阅，订阅编辑页可直接选择整组节点
- 自动订阅任务新增“立即同步”操作，无需等待下一次 Cron 调度
- 恢复自定义服务分流、规则集和 Vodafone Wi-Fi Calling 模板，并加入自动地区分组
- 自建节点提供专属选择与自动优选组，台湾分组使用兼容旗帜图标

### fix
- HTTP/HTTPS 代理节点不再被误判为远程订阅地址
- SQLite 启用 WAL 和写入等待，避免定时同步期间阻塞登录查询
